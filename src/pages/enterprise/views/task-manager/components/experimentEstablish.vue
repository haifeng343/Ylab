    ...
    <div class="app-container">
      <div>
        ...
        <el-form ref="titleForm" :model="editData" :rules="titleRules">
          <el-form-item prop="name">
            <md-input v-model="editData.name" icon="el-icon-search" placeholder="输入实验名称（必填）">
              实验名称
            </md-input>
          </el-form-item>
        </el-form>
      ...
      <div class="container-standard">
        <el-form ref="otherForm" :inline="true" :model="editData" :rules="rules" class="ybr-form-row" label-position="left">
          <!-- <div>
            <el-form-item class="businessContract" label="归属项目:">
          ...
          <el-form-item label="负责部门:" prop="deptId">
            <el-select v-model="editData.deptId" filterable placeholder="请选择">
...
import { fetchPersonList, queryDept } from '@/api/common'
...
export default {
  ...
  created() {
    ...
    fetchContractList({ pageNo: 1, pageSize: 100000 }).then(res => {
      ...
      this.$refs.titleForm.validate((valid) => {
        if (!valid) {
          ...
          if (this.formPass) {
            ...
            this.$emit('taskEdit')
            this.$emit('changeExperimentValidate')
          }
        }
      })
