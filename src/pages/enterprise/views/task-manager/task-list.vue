<template>
  <div class="app-container">
    ...
    <div v-else class="container-line-item">
      ...
      <el-dropdown v-if="buttonPermissions.includes(enterpriseTaskSetting)" trigger="click" class="container-line-button" size="medium">
        <el-button type="primary">
          <div style="height: 12px;">更多<i class="el-icon-arrow-down el-icon--right" /></div>
        </el-button>
        <el-dropdown-menu slot="dropdown">
          <el-dropdown-item @click.native="$router.push('/contract-manager/task-settings')">设置</el-dropdown-item>
          <el-dropdown-item @click.native="$router.push('/contract-manager/batch?type=task')">导入</el-dropdown-item>
    ...
    >
      ...
      <el-table-column v-if="checkedColumns.includes('名称')" label="名称" min-width="260">
        <template slot-scope="{row}">
          <span style="display: inline;" class="link-type" :class="{'finished-item': row.status === 4}" @click="$router.push(`/contract-manager/task-detail?id=${row.id}`)">{{ row.name }}</span>
          ...
          <el-tooltip placement="top">
            ...
            <el-tag v-show="row.exception || row.contractException" type="danger">异常</el-tag>
          </el-tooltip>
      ...
      <el-table-column label="操作" align="center" width="80" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <el-button v-if="buttonPermissions.includes(enterpriseTaskEdit) && row.status === 0 && row.canStart" type="primary" @click="changeStatus(row.id, 1)">
            开始
          </el-button>
          <el-button v-if="row.status === 0 && (!row.canStart || !buttonPermissions.includes(enterpriseTaskEdit))" type="info" disabled>
            开始
          </el-button>
          <el-button v-if="row.status === 1 && !row.isInApprove" type="success" @click="confirmEnd(row.id, 4)">
            完成
          </el-button>
          <el-button v-if="row.status === 3" type="warning" @click="changeStatus(row.id, 1)">
            恢复
          </el-button>
          <el-button v-if="row.status === 4 || (row.status === 1 && row.isInApprove)" type="info" disabled>
            完成
          </el-button>
        </template>
      </el-table-column>
...
const fixedColumns = ['状态', '编号', '名称', '类型', '负责人', '计划开始', '计划结束', '创建时间', '更新时间']
...
export default {
  ...
  computed: {
    ...
    // 任务设置
    enterpriseTaskSetting() {
      return this.buttonPermission[this.platform].enterpriseTaskSetting
    },
    // 开始任务
    enterpriseTaskEdit() {
