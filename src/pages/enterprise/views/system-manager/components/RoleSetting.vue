  ...
  >
    ...
    <div v-show="settingMenuId!==''">
      ...
      <div v-for="dataApproval in dataApprovalList" :key="dataApproval.id" style="display: flex; align-items: center;">
        <el-checkbox :disabled="roleName==='管理员' || roleName === '实验员'" :value="tempSelectDataIds.includes(dataApproval.id)" :label="dataApproval.id" @change="checkData(dataApproval.id)">{{ dataApproval.name }}</el-checkbox>
        <span class="settingHover">
          <svg-icon
            v-if="dataApproval.code==='other' && roleName!=='管理员' && roleName !== '实验员'"
            icon-class="setting2"
      ...
      <div v-for="buttonApproval in buttonApprovalList" :key="buttonApproval.id">
        <el-checkbox :disabled="roleName==='管理员' || roleName === '实验员'" :checked="tempSelectDataIds.includes(buttonApproval.id)" :label="buttonApproval.id" @change="checkData(buttonApproval.id)">{{ buttonApproval.name }}</el-checkbox>
      </div>
    </div>
    <div slot="footer" class="dialog-footer">
  ...
  methods: {
    ...
    async initPower() {
      ...
      if (result.code === 200) {
        ...
        if (this.roleName === '管理员' || this.roleName === '实验员') {
