<template>
  <div v-loading="loading">
      ...
      <div class="container-standard">
        <div class="container-line-title">
          ...
          <el-dropdown trigger="click" class="container-line-button">
            ...
            <el-dropdown-menu slot="dropdown">
              ...
              <el-dropdown-item v-if="$store.state.user.company.isDouble" @click.native="setGuige = true">设置规格</el-dropdown-item>
        ...
        <div class="container-line-item">
          ...
          <span class="detail-form-item">
            ...
            <span class="detail-form-value">{{ detailData.specs }}<span v-if="detailData.useSpecs">(规格2：{{ detailData.useSpecs }})</span></span>
          </span>
          <span class="detail-form-item">
            <span class="detail-form-name">数量：</span>
            <span class="detail-form-value">
              <span v-if="Number.isInteger(detailData.storeCount)">{{ detailData.storeCount }}</span>
              <span v-else>{{ detailData.storeCount.toFixed(2) }}</span>
              <span v-if="Number.isInteger(detailData.useCount)">(规格2：{{ detailData.useCount }})</span>
              <span v-else-if="detailData.useCount">(规格2：{{ detailData.useCount.toFixed(2) }})</span>
            </span>
          </span>
          <span class="detail-form-item">
    ...
    <!-- ------------------------------------------------------设置规格-------------------------------------------- -->
    <el-dialog title="设置规格弹窗" :visible.sync="setGuige" :close-on-click-modal="false" width="440px">
      <el-form :model="receive" label-position="left" label-width="80px">
        ...
        <el-form-item
          label="规格2"
          :rules="[
            { required: true, message: '规格2必须填'}
          ]"
        >
          <el-input v-model="setSpecs.useSpecs" placeholder="输入规格定义" :disabled="detailData.useSpecs ? true : false" />
        </el-form-item>
        <el-form-item
          label="换算"
          :rules="[
            { required: true, message: '换算必须填'}
          ]"
        >
          <div style="display: flex;">
            <span style="width: 72px;">规格1=&nbsp;</span>
            <el-input v-model="setSpecs.conversion" type="number" :disabled="detailData.useSpecs ? true : false" />
            <span style="width: 72px;">&nbsp;规格2</span>
          </div>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="setGuige = false">取 消</el-button>
        <el-button type="primary" @click="handleSpecs">确 定</el-button>
      </div>
    </el-dialog>
    <!-- ------------------------------------------------------删除弹窗-------------------------------------------- -->
    <!-- <el-dialog title="删除物品" :visible.sync="showDel" :close-on-click-modal="false" width="440px">
      <div>确认删除该库存物品？该操作不可恢复！</div>
      <div slot="footer" class="dialog-footer">
        <el-button @click="cancelReceive">取 消</el-button>
        <el-button type="primary" @click="handleReceive">确 定</el-button>
      </div>
...
import { queryStoreDetail, addStoreLog, storeReceive, delBuyStoreData, editStore } from '@/api/store'
...
export default {
  ...
  methods: {
    ...
    async storeDetail() {
      ...
      if (detailRes.data.buyInfo.useSpecs) {
        this.selectSpecs[1] = detailRes.data.buyInfo.useSpecs
        this.setSpecs.useSpecs = detailRes.data.buyInfo.useSpecs
      }
      if (detailRes.data.buyInfo.conversion) {
        this.setSpecs.conversion = detailRes.data.buyInfo.conversion
      }
    },
    cancelHandle() {
      this.newStore = false
    ...
    async handleSpecs() {
      if (!this.setSpecs.useSpecs) {
        this.$message.error('请输入规格2')
        return false
      }
      if (!this.setSpecs.conversion) {
        this.$message.error('请输入换算')
        return false
      }
      const setSpecs = this.setSpecs
      setSpecs.id = this.$route.query.id
      const result = await editStore(setSpecs)
      if (result.code === 200) {
    ...
    delStoreProduct() {
      this.$confirm('确认删除该库存物品？该操作不可恢复！', '删除物品', {
        confirmButtonText: '确定',
        cancelButtonText: '取消'
      }).then(() => {
        delBuyStoreData({ id: this.$route.query.id }).then(res => {
          this.$message({
            type: 'success',
            message: '删除成功!'
          })
