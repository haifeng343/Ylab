<template>
  <div class="app-container">
    ...
    <div class="container-line-item">
      <span class="status-text-button">领用记录</span>
    </div>
    <el-table
      :key="tableKey"
    ...
    >
      ...
      <el-table-column label="状态" class-name="status-col" width="80" align="center">
        <template slot-scope="{row}">
          <el-tag v-if="row.approveStatus === null" type="danger">已退货</el-tag>
          <el-tag v-else :type="statusColor[row.approveStatus]">{{ permitStatus[row.approveStatus] }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="编号" align="center" width="140">
        <template slot-scope="{row}">
          <span>{{ row.buyInfoNo }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="领用数量" align="center" width="120">
        <template slot-scope="{row}">
          <span>{{ row.count }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" width="150" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <el-button v-if="row.type === 5 && row.approveStatus === 1 && row.canApprove || row.type === 5 && row.approveStatus === 1 && row.canApproveBuyBack" type="primary" @click="approveStore(row)">
            审批
          </el-button>
          ...
          <el-button v-if="row.approveStatus !== 1 && row.approveStatus !== 4" type="info" disabled>
            取消
          </el-button>
          <el-button v-if="row.approveStatus === 4" type="primary" @click="sendBack(row)">
            退回
          </el-button>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    // 退回
    sendBack(backProduct) {
      this.$confirm('确定退回此物品？', '退回物品', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        storeReceivePermit({ id: backProduct.id, status: 1 }).then(res => {
          if (res.code === 200) {
            this.$message.success('退回申请成功，请等待审核')
            this.getList()
          }
        })
      }).catch(() => {
