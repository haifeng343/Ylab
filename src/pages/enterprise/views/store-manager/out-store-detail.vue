    ...
    <div class="app-container">
      ...
      <div class="container-line-title">
        ...
        <el-dropdown trigger="click" class="container-line-button">
          ...
          <el-dropdown-menu slot="dropdown">
            ...
            <el-dropdown-item v-if="editData.type === 5 && editData.approveStatus === 1 && (canApproveBuy || canApproveBuyBack)" @click.native="approveStore(editData.id)">审批</el-dropdown-item>
      ...
      <div class="container-line-item">
        <span class="detail-form-item" style="width: 500px;">
          ...
          <span class="detail-form-value link-type" @click="$router.push(`/store-manager/store-detail?id=${editData.storeId}`)">{{ editData.storeNo }} {{ editData.name }}</span>
        </span>
        <span class="detail-form-item" style="width: 500px;">
      ...
      <!-- -----------------------------------------审批流程----------------------------------------- -->
      <div v-if="editData.type !== 6">
        <div class="container-line-title" style="margin-top:20px;">
          <span class="detail-subtitle">审批流程</span>
        </div>
        <el-tabs v-model="activeApproveName" style="margin-top:15px;" type="border-card">
          <el-tab-pane label="领用审批流程" name="outStoreApprove">
          ...
          <el-tab-pane label="领用退回流程" name="backApprove">
            <div v-if="backFlows" style="padding: 24px 0;">
              <el-steps v-if="backFlows" :active="1" space="200px" :align-center="true">
                <el-step v-for="(item,index) in backFlows" :key="index" icon="el-icon-edit">
                  <template slot="icon">
                    <div class="buyApproveNode">
                      <svg-icon v-if="item.status === 4" icon-class="sucessApprove" />
                      <svg-icon v-else-if="item.status === 3" icon-class="failApprove" />
...
export default {
  ...
  data() {
    return {
      ...
      reverse: false,
      activeApproveName: 'outStoreApprove',
      approveFlows: [],
      backFlows: [],
      activities: []
    }
  ...
  methods: {
    ...
    async outStoreDetail() {
      ...
      this.approveFlows = detailRes.data.flows
