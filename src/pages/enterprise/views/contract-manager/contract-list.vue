  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column v-if="checkedColumns.includes('名称')" label="名称" min-width="260">
        <template slot-scope="{row}">
          <span v-if="row.taskException && row.taskException.length > 0 || row.contractException" class="link-type" :class="{'finished-item': row.status === 4}" @click="$router.push(`/contract-manager/contract-detail?id=${row.id}`)">
            {{ row.name }}
            <el-tooltip placement="top">
              <div slot="content">
                <div v-if="row.taskException" style="display: flex;">
                  <div>任务异常：</div>
                  <div>
      ...
      <el-table-column label="操作" align="center" width="80" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <el-button v-if="buttonPermissions.includes(newContract) && row.status === 0" type="primary" @click="changeStatus(row.id, 1)">
            开始
          </el-button>
          <el-button v-if="buttonPermissions.includes(newContract) && row.status === 1 && !row.isInApprove" type="success" @click="confirmEnd(row.id, 4)">
            完成
          </el-button>
          <el-button v-if="buttonPermissions.includes(stopContract) && row.status === 3" type="warning" @click="changeStatus(row.id, 1)">
            恢复
          </el-button>
          <el-button v-if="(buttonPermissions.includes(newContract) && row.status === 4) || (buttonPermissions.includes(newContract) && row.status === 1 && row.isInApprove)" type="info" disabled>
            完成
          </el-button>
        </template>
      </el-table-column>
