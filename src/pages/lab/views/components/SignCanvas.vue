  ...
  methods: {
    ...
    setLineWidth() {
      ...
      if (this.config.isSign) {
        ...
        this.context.lineWidth = this.config.lastWriteWidth =
    ...
    canvasClear() {
      ...
      if (this.config.isShowBorder) {
        ...
        this.context.lineTo(
          this.config.canvasWidth - size,
          this.config.canvasHeight - size
        )
    ...
    saveAsImg() {
      ...
      let oWidth = this.canvas.width
      let oHeight = this.canvas.height
      let nWidth = oHeight
      let nHeight = oWidth
      canvas.width = nWidth
      canvas.height = nHeight
      let oImageDataData = this.canvas
        .getContext('2d')
        .getImageData(0, 0, oWidth, oHeight).data
      let nImageData = canvas
        .getContext('2d')
        .getImageData(0, 0, nWidth, nHeight)
      let nImageDataData = nImageData.data
      let signed = false
      for (let y = 0; y < nHeight; y++) {
        for (let x = 0; x < nWidth; x++) {
          nImageDataData[y * nWidth * 4 + x * 4] =
            oImageDataData[(oHeight - x) * oWidth * 4 + y * 4]
          nImageDataData[y * nWidth * 4 + x * 4 + 1] =
            oImageDataData[(oHeight - x) * oWidth * 4 + y * 4 + 1]
          nImageDataData[y * nWidth * 4 + x * 4 + 2] =
            oImageDataData[(oHeight - x) * oWidth * 4 + y * 4 + 2]
          nImageDataData[y * nWidth * 4 + x * 4 + 3] =
            oImageDataData[(oHeight - x) * oWidth * 4 + y * 4 + 3]
          if (nImageDataData[y * nWidth * 4 + x * 4 + 3] !== 0) {
            signed = true
          }
        }
      }
      if (signed === false) {
        ...
        canvas.getContext('2d').putImageData(nImageData, 0, 0)
