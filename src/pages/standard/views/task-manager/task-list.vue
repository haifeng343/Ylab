  ...
  <div class="app-container">
    ...
    <div v-else class="container-line-item">
      ...
      <el-dropdown v-if="buttonPermissions.includes(taskSetting)" trigger="click" class="container-line-button" size="medium">
        ...
        <el-dropdown-menu slot="dropdown">
          <el-dropdown-item v-if="buttonPermissions.includes(taskSetting)" @click.native="$router.push('/contract-manager/task-settings')">设置</el-dropdown-item>
          <el-dropdown-item v-if="buttonPermissions.includes(taskSetting)" @click.native="$router.push('/contract-manager/batch?type=task')">导入</el-dropdown-item>
          <el-dropdown-item v-if="buttonPermissions.includes(taskSetting)" @click.native="derive">导出</el-dropdown-item>
        </el-dropdown-menu>
      </el-dropdown>
      <el-button class="container-line-button" type="primary" @click="viewColumnEditing=true">
        视图
      </el-button>
      <el-button v-if="buttonPermissions.includes(newTask)" class="container-line-button" type="primary" @click="$router.push('/contract-manager/task-create')">
        新建
      </el-button>
    </div>
    <el-table
    ...
    >
      ...
      <el-table-column v-if="checkedColumns.includes('名称')" label="名称" min-width="260">
        <template slot-scope="{row}">
          <span v-if="!row.h3Id" style="display: inline;" class="link-type" :class="{'finished-item': row.status === 4}" @click="$router.push(`/contract-manager/task-detail?id=${row.id}`)">{{ row.name }}<el-tag v-if="row.isNew === 1" type="danger">NEW</el-tag></span>
          <span v-else style="display: inline;" class="link-type" :class="{'finished-item': row.status === 4}" @click="$router.push(`/contract-manager/cooperate-task-detail?id=${row.id}`)">{{ row.name }} <el-tag v-if="row.h3Id" type="primary">合作</el-tag> <el-tag v-if="row.isNew === 1" type="danger">NEW</el-tag></span>
      ...
      <el-table-column label="操作" align="center" width="80" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <el-button v-if="buttonPermissions.includes(newTask) && row.status === 0 && !row.h3Id" type="primary" @click="changeStatus(row.id, 1)">
            开始
          </el-button>
          <el-button v-if="buttonPermissions.includes(newTask) && row.status === 0 && row.h3Id" type="info" disabled>
            开始
          </el-button>
          <el-button v-if="buttonPermissions.includes(newTask) && row.status === 1 && !row.h3Id" type="success" @click="confirmEnd(row.id, 4)">
            完成
          </el-button>
          <el-button v-if="buttonPermissions.includes(newTask) && row.status === 1 && row.h3Id" type="info" disabled>
            完成
          </el-button>
          <el-button v-if="buttonPermissions.includes(newTask) && row.status === 4" type="info" disabled>
            完成
          </el-button>
          <el-button v-if="buttonPermissions.includes(stopTask) && row.status === 3 && !row.h3Id" type="warning" @click="changeStatus(row.id, 1)">
            恢复
          </el-button>
          <el-button v-if="buttonPermissions.includes(stopTask) && row.status === 3 && row.h3Id" type="info" disabled>
            恢复
          </el-button>
        </template>
      </el-table-column>
...
<script>
import { fetchList, editTaskStatus } from '@/api/task'
...
export default {
  ...
  methods: {
    ...
    changeStatus(id, statusNum) {
      editTaskStatus({ id, status: statusNum }).then(res => {
        this.getList()
      })
    },
    // 确认结束
