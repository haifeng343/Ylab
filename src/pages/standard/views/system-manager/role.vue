    ...
    <el-table v-loading="listLoading" :data="roleData" border style="width: 100%">
      ...
      <el-table-column label="操作" align="center" width="256">
        <template slot-scope="{row}">
          ...
          <el-button v-if="row.name==='管理员'||row.name==='实验员'||row.name==='采购主管'||row.name==='库存主管'" size="mini" type="info" disabled>删除</el-button>
    ...
    >
      ...
      <el-form ref="roleForm" :model="roleForm" :rules="rules" label-position="left" class="ybr-form-col">
        <el-form-item label="角色名称" prop="name">
          <el-input v-model="roleForm.name" :disabled="currentRole.name==='管理员' || currentRole.name==='实验员' || currentRole.name==='采购主管' || currentRole.name==='库存主管'" />
        </el-form-item>
        <el-form-item label="角色说明" prop="remark">
          <el-input v-model="roleForm.remark" :disabled="currentRole.name==='管理员' || currentRole.name==='实验员' || currentRole.name==='采购主管' || currentRole.name==='库存主管'" />
        </el-form-item>
      </el-form>
