  ...
  <div class="app-container">
    <ylab-hiddenable-container v-show="!viewColumnEditing">
      <el-form v-model="listQuery" label-position="left" :inline="true" class="ybr-form-row" style="width: calc(100% - 80px);">
        ...
        <el-form-item v-if="checkedColumns.includes('名称')" label="合作与自研：">
          <el-select v-model="listQuery.isUnion" filterable clearable placeholder="请选择">
            <el-option label="合作项目" :value="1" />
            <el-option label="自研项目" :value="0" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item v-if="checkedColumns.includes('负责人')" label="负责人：">
          <el-select v-model="listQuery.curId" filterable clearable placeholder="请选择">
    ...
    <div v-else class="container-line-item">
      ...
      <el-dropdown v-if="buttonPermissions.includes(contractSetting)" trigger="click" class="container-line-button" size="medium">
        <el-button type="primary">
          <div style="height: 12px;">更多<i class="el-icon-arrow-down el-icon--right" /></div>
        </el-button>
        <el-dropdown-menu slot="dropdown">
          <el-dropdown-item v-if="buttonPermissions.includes(contractSetting)" @click.native="$router.push('/contract-manager/contract-settings')">设置</el-dropdown-item>
          <el-dropdown-item v-if="buttonPermissions.includes(contractSetting)" @click.native="$router.push('/contract-manager/batch?type=contract')">导入</el-dropdown-item>
          <el-dropdown-item v-if="buttonPermissions.includes(contractSetting)" @click.native="derive">导出</el-dropdown-item>
        </el-dropdown-menu>
      </el-dropdown>
      <el-button class="container-line-button" type="primary" @click="viewColumnEditing=true">
        视图
      </el-button>
      <el-button v-if="buttonPermissions.includes(newContract)" class="container-line-button" type="primary" @click="$router.push('/contract-manager/contract-create')">
        新建
      </el-button>
    </div>
    <el-table
    ...
    >
      ...
      <el-table-column v-if="checkedColumns.includes('编号')" label="编号" width="140" align="center">
        <template slot-scope="{row}">
          <span :class="{'finished-item': row.status === 4}">{{ row.contractNo }}</span>
        </template>
      </el-table-column>
      <el-table-column v-if="checkedColumns.includes('名称')" label="名称" min-width="260">
        <template slot-scope="{row}">
          <span v-if="row.h3Id" class="link-type" :class="{'finished-item': row.status === 4}" @click="$router.push(`/contract-manager/cooperate-contract?id=${row.h3Id}`)">{{ row.name }} <el-tag type="primary">合作</el-tag></span>
          <span v-else class="link-type" :class="{'finished-item': row.status === 4}" @click="$router.push(`/contract-manager/contract-detail?id=${row.id}`)">{{ row.name }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column v-if="checkedColumns.includes('负责人')" label="负责人" align="center" width="110">
        <template slot-scope="{row}">
      ...
      <el-table-column label="操作" align="center" width="80" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <el-button v-if="buttonPermissions.includes(newContract) && row.status === 0 && !row.h3Id" type="primary" @click="changeStatus(row.id, 1)">
            开始
          </el-button>
          <el-button v-if="buttonPermissions.includes(newContract) && row.status === 0 && row.h3Id" type="info" disabled>
            开始
          </el-button>
          <el-button v-if="buttonPermissions.includes(newContract) && row.status === 1 && !row.h3Id" type="success" @click="confirmEnd(row.id, 4)">
            完成
          </el-button>
          <el-button v-if="buttonPermissions.includes(newContract) && row.status === 1 && row.h3Id" type="info" disabled>
            完成
          </el-button>
          <el-button v-if="buttonPermissions.includes(newContract) && row.status === 4" type="info" disabled>
            完成
          </el-button>
          <el-button v-if="buttonPermissions.includes(stopContract) && row.status === 3 && !row.h3Id" type="warning" @click="changeStatus(row.id, 1)">
            恢复
          </el-button>
          <el-button v-if="buttonPermissions.includes(stopContract) && row.status === 3 && row.h3Id" type="info" disabled>
            恢复
          </el-button>
        </template>
      </el-table-column>
...
const fixedColumns = ['状态', '编号', '名称', '类型', '优先级', '负责人', '计划开始', '计划结束', '创建时间', '更新时间']
...
export default {
  ...
  computed: {
    ...
    // 项目设置
    contractSetting() {
      return this.buttonPermission[this.platform].contractSetting
    },
    // 新建 编辑项目
    newContract() {
