...
import { getInvoice, getPaying, getJoinContract } from '@/api/user'
import { fetchList } from '@/api/task'
export default {
  name: 'CooperateContract',
  ...
  methods: {
    async initData() {
      this.listQuery.objectId = this.$route.query.id
      this.listLoading = true
      const res = await getJoinContract(this.listQuery)
      if (res.code === 200) {
        this.detail = res.data.map(item => {
          if (item.objectId === this.$route.query.id) {
            return item
          }
        })
